...
// Import upgraded commercial and mobile responsive modules
import MiniScreening from './components/MiniScreening';
import PricingSection from './components/PricingSection';
import PaymentModal from './components/PaymentModal';
import LockedReportSections from './components/LockedReportSections';
import ShareModal from './components/ShareModal';
import { Share2, Lock, Sparkles, HelpCircle, MessageSquare } from 'lucide-react';
...
export default function App() {
  ...
  // Commercial & responsive upgrades states
    ...
    <div className="min-h-screen bg-[#F0F2F5] text-zinc-900 font-sans selection:bg-blue-500/30">
      <AnimatePresence mode="wait">
        ...
        {isScreening ? (
          ...
          >
            <MiniScreening 
              onBack={() => setIsScreening(false)} 
              onStartFullAssessment={() => {
                setIsScreening(false);
                setState(TestState.REGISTRATION);
              }} 
            />
          </motion.div>
          ...
          <motion.div 
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="max-w-6xl mx-auto pt-10 pb-20 px-4 md:px-6 space-y-14"
          >
            {/* Center Registration Box */}
            <div className="max-w-xl mx-auto bg-white border border-zinc-200 shadow-2xl rounded-[2.5rem] overflow-hidden">
              <div className="bg-gradient-to-br from-indigo-700 to-indigo-900 p-10 text-white relative">
                ...
                <p className="mt-2 text-indigo-100/80 font-medium">视听整合连续测试 · 中文专业版</p>
              </div>
              ...
              <div className="p-10 space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl px-5 py-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium h-12 flex items-center"
                      placeholder="请输入姓名"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-1">测试编号</label>
                    <div className="w-full bg-zinc-100 border border-zinc-200 rounded-2xl px-5 py-4 font-mono text-sm text-zinc-500 flex items-center justify-between h-12">
                      {userInfo.testId}
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl px-5 py-3 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all appearance-none font-medium h-12"
                    >
                ...
                {/* Mobile & Quiet Environment checklist warning box */}
                <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-2xl flex gap-4 items-start text-left">
                  <ShieldCheck className="w-5 h-5 text-indigo-600 shrink-0 mt-0.5" />
                  <div className="space-y-1.5">
                    <p className="text-xs text-indigo-900 leading-none font-black">
                      测试环境保障提示 (Environment Checklist)
                    </p>
                    <p className="text-[11px] text-indigo-805 text-indigo-800 leading-normal font-semibold">
                      本临床测试包含 13 分钟的高精密连续反应刺激。为保障报告数据真实可靠：
                    </p>
                    <ul className="list-disc pl-4 text-[10px] text-indigo-700/90 space-y-1 font-bold">
                      <li>确保独处在极其安静、无外部声影干扰的房间</li>
                      <li>手机端用户请关闭各种通知（建议开启“勿扰模式”）</li>
                      <li>请配戴高品质耳机，并将系统音量调至恰当级别</li>
                      <li>看到或听到 1 时请迅速敲击 <strong>空格键</strong> (手机上则请点击屏幕任意位置)</li>
                    </ul>
                  </div>
                </div>

                {/* Double finger / thumb height compatibility button */}
                <button 
                  onClick={() => setState(TestState.INSTRUCTIONS)}
                  disabled={!userInfo.name || !userInfo.age}
                  className="w-full bg-zinc-900 hover:bg-black text-white font-bold py-5 rounded-[1.5rem] shadow-xl shadow-zinc-900/20 hover:shadow-black/30 transition-all flex items-center justify-center gap-3 disabled:opacity-30 h-14 active:scale-95 cursor-pointer select-none"
                >
                  进入评估流程 <Play className="w-4 h-4" />
                </button>
              </div>
            </div>

            {/* Premium strata services Pricing matrix section */}
            <div id="registration_pricing_section" className="w-full border-t border-zinc-200/60 pt-12">
              <PricingSection 
                onSelectTier={(tier) => {
                  if (tier === 'screening') {
                    setIsScreening(true);
        ...
        {(state === TestState.TESTING || state === TestState.PRACTICE) && (
          <motion.div 
            ...
            className="fixed inset-0 bg-[#0A0A0B] flex flex-col items-center justify-center overflow-hidden cursor-pointer select-none"
          >
            ...
            <div className="absolute bottom-16 text-zinc-600 text-xs font-bold uppercase tracking-[0.2em] text-center max-w-lg leading-relaxed select-none">
              看到或听到 "1" 时请迅速按 <span className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">空格键</span> 或 <span className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">轻触屏幕任意位置</span><br/>
              看到或听到 "2" 时请 <span className="text-zinc-500 font-extrabold underline">保持绝对静止</span>
            </div>
          </motion.div>
        )}
        ...
        {state === TestState.RESULTS && results && (
          ...
          >
            ...
            <div className="bg-white border-2 border-zinc-900 shadow-2xl p-10 print:shadow-none print:border-none print:p-0">
              ...
              <div className="flex justify-between items-end border-b-4 border-zinc-900 pb-10 mb-10">
                ...
                <div className="text-right flex flex-col items-end">
                  ...
                  <button
                    id="btn_share_top_report"
                    onClick={() => setIsShareOpen(true)}
                    className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black rounded-xl cursor-pointer transition-all shadow-md active:scale-95 no-print"
                  >
                    <Share2 className="w-3.5 h-3.5" /> 一键分享报告
                  </button>
                </div>
              </div>
                ...
                <div className={cn(
                  ...
                  <div className="flex gap-10 items-start">
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="pt-4 border-t border-zinc-300 grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              {/* Recommendations */}
              <div className="mb-10">
                 {/* Interactive monetization CTA for full analysis & expert consultation */}
                 <div id="diagnostic_upgrade_cta" className="mb-10 p-6 bg-zinc-50 border-2 border-zinc-900 rounded-sm flex flex-col sm:flex-row items-center justify-between gap-4 no-print text-left">
                   <div className="flex items-center gap-2">
                     <Sparkles className="w-5 h-5 text-indigo-600 animate-pulse shrink-0 animate-bounce" />
                     <div className="space-y-0.5">
                       <span className="text-xs font-black text-zinc-900 block">对测试数据与指标仍保留疑问？</span>
                       <span className="text-[11px] text-zinc-500 font-bold leading-normal block">需要制定个性化日常专注特训干预指导，或获取就医、训练避坑建议：</span>
                     </div>
                   </div>
                   <button
                     id="btn_diagnosis_interpretation_monetization"
                     onClick={() => setIsPaymentOpen(true)}
                     className="px-5 py-3 bg-zinc-900 hover:bg-black text-white text-xs font-black rounded-xl transition-all shadow-md active:scale-95 cursor-pointer inline-flex items-center gap-1.5 shrink-0 select-none"
                   >
                     ✨ 申请专家 1v1 报告精讲与行为干预
                   </button>
                 </div>
            ...
            {/* Locked report premium extensions (Trend analysis Tracker & Consultation Booking) */}
            <div className="mt-12 no-print space-y-12">
              <div className="text-center">
                <span className="text-[10px] uppercase font-black tracking-widest text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">Premium Addons</span>
                <h2 className="text-3xl font-black tracking-tight text-zinc-900 mt-2 mb-1.5">拓展你的专注力管理</h2>
                <p className="text-xs text-zinc-500 font-bold">获取多维度的追踪与指导，加速个人脑潜力整合提升</p>
              </div>
              <LockedReportSections onUnlockClick={() => setIsPaymentOpen(true)} />
            </div>

            {/* Bottom complete Pricing matrix again for transparency & easy upgrades */}
            <div className="mt-14 border-t border-zinc-200/60 pt-12 no-print">
              <PricingSection 
                onSelectTier={(tier) => {
                  if (tier === 'screening') {
                    setIsScreening(true);
            ...
            <div className="flex justify-center gap-6 mt-12 no-print">
               <button 
                ...
                className="flex items-center gap-3 px-10 py-4.5 bg-zinc-900 text-white rounded-[2rem] font-black hover:bg-black transition-all shadow-xl shadow-zinc-900/30 active:scale-95 cursor-pointer select-none"
              >
                <Printer className="w-4 h-4 font-bold" /> PRINTER 打印这份报告
              </button>
              <button 
                onClick={() => {
                  // Reset states
                ...
                className="flex items-center gap-3 px-10 py-4.5 bg-white border-2 border-zinc-900 text-zinc-900 rounded-[2rem] font-black hover:bg-zinc-50 transition-all active:scale-95 cursor-pointer select-none"
              >
                <RefreshCcw className="w-4 h-4" /> RESTART 重新测试
              </button>
            </div>
          </motion.div>
        )}
      ...
      {/* Overlays / Pop-ups Overlay mounting */}
      <PaymentModal 
        isOpen={isPaymentOpen} 
        onClose={() => setIsPaymentOpen(false)} 
        onUnlockFullReport={() => {
          // It's already free and visible, so we let them know and scroll back nicely!
          window.scrollTo({ top: 0, behavior: 'smooth' });
        }}
      />
      ...
      <ShareModal 
        ...
        userData={{
          ...
          testDate: userInfo.testDate || new Date().toISOString(),
          riskLevel: results ? results.classification : '温和',
          summaryText: results ? results.clinicalNote : '',
          preference: results ? results.visualAuditoryBalance : '视听均衡'
        }}
      />
      
      {/* Floating sticky bottom bar on mobile screens (width <= 767px) for high converting share/expert path */}
      <AnimatePresence>
        {state === TestState.RESULTS && results && (
          <motion.div
            ...
            exit={{ y: 80, opacity: 0 }}
            className="md:hidden fixed bottom-4 inset-x-4 h-16 bg-zinc-900/95 backdrop-blur-md rounded-2xl z-40 px-4 py-2 flex items-center justify-between shadow-2xl border border-white/10 no-print"
          >
            <div className="flex flex-col text-left">
              <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider leading-none mb-1">注意力状态倾向</span>
              <span className="text-xs font-black text-white leading-none">{results.diagnosis}</span>
            </div>
            
            <div className="flex gap-2">
              <button
                id="mobile_sticky_share_btn"
                onClick={() => setIsShareOpen(true)}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black rounded-lg flex items-center gap-1.5 shadow-md active:scale-95 transition-all cursor-pointer"
              >
                <Share2 className="w-3.5 h-3.5" /> 一键分享
              </button>
              
              <button
                id="mobile_sticky_pay_btn"
                onClick={() => setIsPaymentOpen(true)}
                className="px-3.5 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-black rounded-lg flex items-center gap-1 active:scale-95 transition-all cursor-pointer animate-pulse"
              >
                咨询专家
              </button>
            </div>
